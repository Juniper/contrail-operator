	...
	t.Run("given contrail-operator is running", func(t *testing.T) {
		...
		t.Run("when manager is updated with swift service", func(t *testing.T) {
			...
			t.Run("then a SwiftProxy pod should be created", func(t *testing.T) {
				swiftProxyPods, err := f.KubeClient.CoreV1().Pods("contrail").List(context.Background(), meta.ListOptions{
		...
		t.Run("when swift file is uploaded", func(t *testing.T) {
			var (
				...
				swiftProxy        = proxy.NewSecureClientForService("contrail", "openstacktest-swift-proxy-swiftproxy", 5070)
