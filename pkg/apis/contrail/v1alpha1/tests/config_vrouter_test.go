...
func TestVrouterConfigStaticConfiguration(t *testing.T) {
	...
	if environment.vrouterConfigMap.Data["vrouter.1.1.8.1"] != vrouterConfigStaticConfigurationDefaultPorts {
		configDiff := diff.Diff(environment.vrouterConfigMap.Data["vrouter.1.1.8.1"], vrouterConfigStaticConfigurationDefaultPorts)
		t.Fatalf("get vrouter config: \n%v\n", configDiff)
	}
}
