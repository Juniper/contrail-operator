...
QUERY_RESULT=$(psql -w -h {{ .PostgresIP }} -U {{ .PostgresUser }} -d {{ .PostgresDBName }} -tAc "SELECT EXISTS (SELECT 1 FROM node LIMIT 1)")
QUERY_EXIT_CODE=$?
...
set -e
psql -w -h {{ .PostgresIP }} -U root -d contrail_test -f /usr/share/contrail/gen_init_psql.sql
psql -w -h {{ .PostgresIP }} -U {{ .PostgresUser }} -d {{ .PostgresDBName }} -f /usr/share/contrail/init_psql.sql
